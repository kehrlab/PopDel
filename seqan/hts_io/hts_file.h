  ...
  public:
    ...
    open()
    {
        ...
        if (fp == nullptr)
        {
            ...
            msg << "Could not open sequence file with filename \'" << filename << ".\' Terminating.";
    ...
    inline bool
    open(const char * referenceFile)
    {
        const char * read_mode = "r";
        fp = hts_open(filename, file_mode);
        if (referenceFile && fp->is_cram)
        {
            const char * s("reference=");
            char * o =  static_cast<char*>(calloc(strlen(s) + strlen(referenceFile) + 1, sizeof(char)));
            strcat(o, s);
            strcat(o, referenceFile);
            hts_opt *opts = NULL;
            hts_opt_add(&opts, o);
            free(o);
            if (hts_opt_apply(fp, opts) < 0)
            {
                std::ostringstream msg;
                msg << "Could not open reference file with filename \'" << referenceFile << "\'. Terminating.";
                SEQAN_THROW(IOError(toCString(msg.str())));
            }
        }
...
/**
 * @brief Opens a HTS file from filename and reference file.
 *
 * @param target An empty HtsFile object.
 * @param f The filename to open from.
 * * @param r The filename of the reference file.
 */
inline bool
open(HtsFile & target, const char * f, const char * r)
{
    target.filename = f;
    return target.open(r);
}
...
/**
 * @brief Reads an alignment record without the sequence or qualities.
 *
 * @param record The record to insert data into.
 * @param file The file to read from.
 */
inline bool
seqFreeReadRecord(BamAlignmentRecord & record, HtsFile & file)
{
    if (readRecord(file))
    {
        seqFreeParse(record, file.hts_record);
        return true;
    }
    else
    {
...
// Read the regions of the alignment file, but without parsing the sequence
inline bool
seqFreeReadRegion(BamAlignmentRecord & record, HtsFile & file)
{
    if (file.read_all)
    {
        return seqFreeReadRecord(record, file);
    }
    int res = sam_itr_next(file.fp, file.hts_iter, file.hts_record);
    if (res >= 0)
    {
        seqFreeParse(record, file.hts_record);
        return true;
    ...
    {
        std::ostringstream msg;
        msg << "[PopDel] An error occured while reading the sequence file (see above).";
        if (file.fp->is_cram)
        {
            msg << " Please make sure that the CRAM header entries of the @SQ lines point to the correct"
                << " reference file(s) or specify the correct reference file using the option \'-R\'.";
        }
        msg << " Terminating.";
        SEQAN_THROW(IOError(toCString(msg.str())));
        return false;
